...
type SectionHeadingProps = {
  ...
  showDot?: boolean;
  /** Render for a dark band (white heading, lighter subtext). */
  tone?: "light" | "dark";
  className?: string;
};
...
export default function SectionHeading({
  ...
  return (
    ...
    >
      {overline && (
        <p className="mb-3 font-sans text-xs font-semibold uppercase tracking-[0.2em] text-brand-gold sm:text-sm">
          {overline}
        </p>
      )}
      ...
      <h2
        className={
          ...
          (onDark ? "text-brand-white" : "text-brand-charcoal")
        }
      >
        {title}
      ...
      {subtext && (
        <p
          className={
            "mt-5 font-sans text-base md:text-lg leading-relaxed " +
            (onDark ? "text-white/80" : "text-brand-grey")
          }
        >
          {subtext}
        </p>
